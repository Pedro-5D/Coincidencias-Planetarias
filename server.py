...
def calculate_fardaria_periods(birth_date, is_dry):
    """Calcula los periodos de Fardarias para una fecha de nacimiento hasta 84 años."""
    planet_order = PLANET_ORDER['seco'] if is_dry else PLANET_ORDER['humedo']
    all_periods = []
    current_day = 0
    total_years = 0
    max_years = 84  # Limitar a 84 años
    
    # Asegurar que birth_date es un objeto datetime
    if isinstance(birth_date, str):
        birth_date = datetime.strptime(birth_date, '%Y-%m-%d')
    
    # Continuar calculando periodos hasta llegar a 84 años
    while total_years < max_years:
        for planet in planet_order:
            duration = calculate_duration(planet, 1)
            years_in_period = duration / DURACIONES['AÑO']
            
            # Verificar si añadiendo este periodo excederíamos los 84 años
            if total_years + years_in_period > max_years:
                # Calcular cuántos días podemos incluir sin exceder 84 años
                remaining_years = max_years - total_years
                remaining_days = remaining_years * DURACIONES['AÑO']
                
                if remaining_days > 0:
                    start_date = calculate_date(birth_date, current_day)
                    end_date = calculate_date(birth_date, current_day + remaining_days)
                    ...
                    period = {
                        ...
                        'end': end_date.strftime('%Y-%m-%d'),
                        'startDay': current_day,
                        'durationDays': remaining_days
                    }
                    
                    # Añadir subperiodos para el tiempo restante
                    period['subPeriods'] = calculate_sub_periods(
                        planet,
                        2,
                        current_day,
                ...
                # Salir después de añadir el periodo parcial
                return all_periods
...
def calcular_relevodPeriods(fecha_nac, ascendente):
    """Calcula periodos de relevo zodiacal hasta 84 años."""
    secuencia = generar_secuencia(ascendente)
    dia_actual = 0
    periodos = []
    anos_totales = 0
    max_anos = 84  # Limitar a 84 años
    ...
    # Continuar calculando periodos hasta llegar a 84 años
    while anos_totales < max_anos:
        for signo in secuencia:
            dias_en_periodo = DURACION_POR_NIVEL[signo] * DURACIONES['AÑO']
            anos_en_periodo = DURACION_POR_NIVEL[signo]
            
            # Verificar si añadiendo este periodo excederíamos los 84 años
            if anos_totales + anos_en_periodo > max_anos:
                # Calcular cuántos días podemos incluir sin exceder 84 años
                anos_restantes = max_anos - anos_totales
                dias_restantes = anos_restantes * DURACIONES['AÑO']
                
                if dias_restantes > 0:
                    fecha_inicio = calculate_date(fecha_nac, dia_actual)
                    fecha_fin = calculate_date(fecha_nac, dia_actual + dias_restantes)
                    ...
                    periodo = {
                        ...
                        'startDay': dia_actual,
                        'durationDays': dias_restantes
                    }
                    
                    # Calcular subperiodos para el tiempo restante
