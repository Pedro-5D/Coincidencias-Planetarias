document.addEventListener('DOMContentLoaded', function() {
    ...
    const DIMENSIONS = {
        ...
        centerY: 300,
        radius: 250,
        middleRadius: 180, // Radio para la carta externa
    ...
    const TIMEZONEDB_API_KEY = "EPDKPLKPJB7G"; // Clave para TimeZoneDB
    ...
    async function calculateChart() {
        // Validar entradas
        if (!natalCityInput.value || !natalDateInput.value || !natalTimeInput.value || !natalCityCoordinates) {
            showError("Debes ingresar ciudad, fecha y hora para la carta natal.");
            return;
        }
        
        if (showTransitsToggle.checked && (!transitCityInput.value || !transitDateInput.value || !transitTimeInput.value || !transitCityCoordinates)) {
    ...
    // Funciones para calcular Fardarias y Relevos Zodiacales
    ...
    // Simular cálculo de posiciones planetarias
    ...
    function init() {
        ...
        // Event listeners
        natalCityInput.addEventListener('input', handleNatalCityInput);
        transitCityInput.addEventListener('input', handleTransitCityInput);
    ...
    // Función para manejar la búsqueda de ciudades natales
    function handleNatalCityInput() {
        const searchText = natalCityInput.value.trim();
        
        if (searchText.length < 3) {
            natalCityResults.innerHTML = '';
            natalCityResults.classList.add('hidden');
            return;
        }
        ...
        const cities = [
            { value: `${searchText}, España`, label: `${searchText}, España`, lat: 40.416, lon: -3.703 },
            { value: `${searchText}, México`, label: `${searchText}, México`, lat: 19.432, lon: -99.133 },
            { value: `${searchText}, Argentina`, label: `${searchText}, Argentina`, lat: -34.603, lon: -58.381 },
            { value: `${searchText}, Estados Unidos`, label: `${searchText}, Estados Unidos`, lat: 37.090, lon: -95.712 },
            { value: `${searchText}, Brasil`, label: `${searchText}, Brasil`, lat: -15.826, lon: -47.921 }
        ];
        
        displayCityResults(cities, natalCityResults, selectNatalCity);
    }
    
    // Función para manejar la búsqueda de ciudades de tránsito
    function handleTransitCityInput() {
        const searchText = transitCityInput.value.trim();
        
        if (searchText.length < 3) {
            transitCityResults.innerHTML = '';
            transitCityResults.classList.add('hidden');
            return;
        }
        ...
        displayCityResults(cities, transitCityResults, selectTransitCity);
    }
    ...
    function calculateChart() {
        ...
        try {
            ...
            // Simular cálculo de ascendente (en una aplicación real usaríamos cálculos astronómicos)
            const ascData = {
                longitude: 0, // Simplificado para este ejemplo
                sign: SIGNS[0].name
            };
            
            // Calcular posiciones planetarias (simuladas)
            ...
            if (ascPosition && sunPosition) {
                ...
                // Calcular coincidencias (simplificado)
                coincidencias = generateMockCoincidencias();
            }
    ...
    function generateMockCoincidencias() {
        const now = new Date();
        const currentYear = now.getFullYear();
        
        const coincidencias = [];
        
        // Generar coincidencias para el año actual y los siguientes 2 años
        for (let year = currentYear; year <= currentYear + 2; year++) {
            const numCoincidencias = 2 + Math.floor(Math.random() * 3); // 2-4 coincidencias por año
            
            for (let i = 0; i < numCoincidencias; i++) {
                const planet = Object.keys(PLANET_DATA)[Math.floor(Math.random() * Object.keys(PLANET_DATA).length)];
                const signo = Object.keys(SIGNOS)[Math.floor(Math.random() * Object.keys(SIGNOS).length)];
